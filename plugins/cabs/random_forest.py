...
class CabsRandomForest(Plugin):
    description = '''
    Predicting cabs patient outcome using random forest method
    '''

    scheme_sample: dict
    scheme_prediction: dict
    plugin_dir = 'cabs_random_forest'
    ...
    def save_in_file(self) -> str:
        path = f'{CabsRandomForest.plugin_dir}/{str(uuid4())}'
    ...
    def on_load() -> None:
        if not exists(f'{directory()}/{CabsRandomForest.plugin_dir}'):
            makedirs(f'{directory()}/{CabsRandomForest.plugin_dir}')

        with open('./plugins/cabs/schemas/data_schema.json', 'r') as file:
            CabsRandomForest.scheme_sample = json.load(file)

        with open('./plugins/cabs/schemas/outcome_schema.json', 'r') as file:
            CabsRandomForest.scheme_prediction = json.load(file)
