...
class CabsDecisionTree(Plugin):
    description = '''
    Predicting cabs patient outcome using decision tree method
    '''

    scheme_sample: dict
    scheme_prediction: dict
    plugin_dir = 'cabs_decision_tree'
    ...
    def save_in_file(self) -> str:
        path = f'{CabsDecisionTree.plugin_dir}/{str(uuid4())}'
    ...
    def on_load() -> None:
        if not exists(f'{directory()}/{CabsDecisionTree.plugin_dir}'):
            makedirs(f'{directory()}/{CabsDecisionTree.plugin_dir}')

        with open('./plugins/cabs/schemas/data_schema.json', 'r') as file:
            CabsDecisionTree.scheme_sample = json.load(file)

        with open('./plugins/cabs/schemas/outcome_schema.json', 'r') as file:
            CabsDecisionTree.scheme_prediction = json.load(file)
